...
\pnote{
  ... validating basic research on
  multi-UAV swarms and formations...

  ... difficult do conduct without proper UAV system ...
}
...
\begin{itemize}
  ...
  \item The fastest landing among all the teams (\SI{25}{\second})
...
\column{0.48\textwidth} % Right column and width
  ...
  \begin{block}{\small Aerial swarming (GPS-denied)}
  \begin{center}
    \mymovie[autostart,loop]{
      \includegraphics[width=0.9\textwidth]{./videos/forest_swarm_short_thumbnail.jpg}
    }{./videos/forest_swarm_short.mp4}\\
  \end{center}
\end{block}
...
\column{0.48\textwidth} % Left column and width
  ...
  \begin{block}{Mine exploration (Finals, 2021)}
    \begin{center}
      \mymovie[autostart,loop]{
...
\column{0.48\textwidth} % Right column and width
  ...
  \begin{block}{Virtual cave exploration (Finals, 2021)}
  \begin{center}
    \mymovie[autostart,loop]{
...
\begin{block}{UAV Experiment in SUJCHBO}
  \begin{center}
    \mymovie[autostart,loop]{
      \includegraphics[width=0.90\textwidth]{./videos/rospix_uav_fast_thumbnail.jpg}
    }{./videos/rospix_uav_fast.mp4}\\
  \end{center}
\end{block}
...
\begin{frame}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.65\textwidth]{./fig/sketch/compton_3d.pdf}
    \caption{
...
\begin{frame}
\frametitle{Real-world experiment in SUJCHBO}
...
\begin{block}{Video}
  \begin{center}
    \mymovie[autostart,loop]{
      \includegraphics[width=0.95\textwidth]{./videos/radron_sujchbo_trash_thumbnail.jpg}
    }{./videos/radron_sujchbo_trash.mp4}\\
  \end{center}
\end{block}
...
\begin{itemize}
  \item \isotope[137]{Cs} (various activities)
  \item completely autonomous search and tracking
  \item 3D mapping and planning
  \item GNSS and GNSS-denied localization
  \item real-time reporting and visualization
\end{itemize}
...
\begin{block}{Citations}
  \begin{itemize}
    \item \makebox[2.8cm]{WoS w/o autocit:\hfill} 194
    \item \makebox[2.8cm]{WoS all:\hfill} 319
    \item \makebox[2.8cm]{Scopus:\hfill} 517
    \item \makebox[2.8cm]{Google scholar:\hfill} 1198
  \end{itemize}
\end{block}

\begin{block}{H-index}
  \begin{itemize}
    \item \makebox[2.8cm]{WoS w/o autocit:\hfill} 8
    \item \makebox[2.8cm]{WoS all:\hfill} 11
    \item \makebox[2.8cm]{Scopus:\hfill} 12
    \item \makebox[2.8cm]{Google scholar:\hfill} 18
  \end{itemize}
\end{block}
...
\begin{frame}
\frametitle{Discussion --- \emph{Views of the Future}}

\begin{block}{GNSS-enabled environment}
  \begin{itemize}
    \item most challenges are already solved (localization, mapping, planning, control)
    \item airspace is being more and more restricted
    \item impressive results in precise agriculture, monitoring and surveillance
    \item $\implies$ less space for public use, more utilization by industry under controlled conditions
    \item $\implies$ multi-UAV approaches: hard-to-know now, possibly in SAR
  \end{itemize}
\end{block}

\begin{block}{GNSS-denied environment}
  \begin{itemize}
    \item only incremental improvements in localization techniques (SLAMs)
    \item diverse family of exteroceptive sensors (monocular/stereo visible, IR, LiDAR, RADAR)
    \item ? major step in SLAM technology: multimodal approaches
    \item ? multimodal sensors: e.g., LiDAR + RADAR + IR camera
    \item $\implies$ real-world deployment in unstructured environment, SAR
  \end{itemize}
\end{block}
...
\frametitle{Scattering angle reconstruction}
  ...
  \scriptsize 1. converting the measured particle energies from (\si{\kilo\electronvolt}) to (\si{\joul}) as
  \begin{align*}
    \scriptsize E_{e^{\minus}} &= e_{e^{\minus}}/6.242\times10^{18},\\
    \scriptsize E_{\lambda} &= e_{\lambda}/6.242\times10^{18}.
  \end{align*}
  \scriptsize 2. calculating the scattering angle using (\ref{eq:compton_formulae}):
  \begin{align*}
    \scriptsize
    \scriptsize \Theta = \cos^{-1} \underbrace{\left(1 + m_e c^2 \left(\frac{1}{E_{e^{-}} + E_\lambda} - \frac{1}{E_\lambda}\right)\right)}_{B},\\\text{for}\ \minus 1 < B < 1
  \end{align*}
  \scriptsize 3. the cone origin $\mathbf{o}_{\mathcal{C}} = \mathbf{e}^{\minus}$, the direction $\mathbf{\hat{d}}_\mathcal{C} = \mathbf{e}^{\minus} - \lambda'$, and the inner angle $\Theta$.
