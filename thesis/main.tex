...
\begin{itemize}
  \item We proposed an MPC trajectory tracking approach, which allows fast and precise tracking of even infeasible trajectories. The tracker also provides distributed collision avoidance for multiple \acp{UAV} sharing the same outdoor workspace.
  \item We proposed a control architecture that integrates the MPC tracking approach in conjunction with a modified non-linear geometric tracking controller.
  ...
  \item The problems of multi-\ac{UAV} collaborative missions for autonomous object manipulation and delivery were studied in the context of the MBZIRC 2017 and 2020 missions. The proposed systems focusing on visual servoing showed the best results among other teams during the objective experimental evaluation during the competition.
  \item The problem of autonomous \ac{UAV} visual servoing of a moving car and a subsequent landing on the car was also studied in the context of the MBZIRC 2017 competition. The proposed solution also showed excellent performance during the evaluation while achieving the fastest landing among the competing teams.
  ...
  \item A system for the radiation dosimetry using Timepix sensors onboard \acp{UAV} was proposed. The system consists of a real-time Monte-Carlo ray-tracing simulator, software for integrating Timepix sensors onboard a \ac{UAV} and a classification pipeline for real-time track classification. Simulations were conducted showing the potential of the approach.
  \item A Compton camera sensor, based on the Timepix3 event camera, on a \ac{UAV} was first proposed and tested onboard, being the first step in an ongoing project regarding localization of compact sources of ionizing radiation by \acp{UAV}.
\end{itemize}
