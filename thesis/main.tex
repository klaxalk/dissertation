...
The thesis author has made his initial in the field by developing a remote sensing module with the Timepix~\cite{llopart2007timepix} detector for the first Czech CubeSat, the VZLUSAT-1.
...
This thesis aims to push the state-of-the-art by utilizing miniature semiconductor pixel detectors \cite{llopart2007timepix}, and novel event-based radiation detectors \cite{poikela2014timepix3} onboard miniature \acp{UAV}.
...
The Timepix sensor \cite{llopart2007timepix, poikela2014timepix3} is an innovative design of an \ac{ASIC} \ac{CMOS} chip that can be bonded to a variety of semiconductor detection materials (Silicon, CdTe).
...
New radiation data are being processed on regular basis while being added to an open-source dataset\footnote{\url{https://github.com/klaxalk/vzlusat-radiation-data}} potentially useful for the community of future CubeSat designers.
...
The first core publication in this thesis, presented at the IEEE IROS 2018 conference, proposes a model predictive tracking mechanism for fast generation of feasible control references for \acp{UAV}.
\fullciteinbox{baca2018model}{}
\noindent
Obtaining a feasible control reference that satisfies state constraints of the \ac{UAV} is vital requirement for fast and precise feedback control.
Moreover, the presented approach utilizes the \SI{8}{s} \ac{MPC} prediction horizon for a mutual \ac{UAV} collision avoidance, that is especially useful for the verification of new multi-\ac{UAV} methods.
The presented control technique was partially responsible for our success in the MBZIRC 2018 robotic competition \cite{baca2019autonomous, loianno2018localization, spurny2019cooperative}.
...
This open-source control system sets up novel paradigms, such as the multi-frame \ac{UAV} state estimation, and the heading-based control pipeline, devout of Euler and Tait-Bryan angles.
Testing of new feedback controllers, reference generators, state estimators and other parts of the control pipeline are possible thanks to the modular design and the extensive simulation environment.
...
However, most the author's effort during his studies had been invested to the research scenarios proposed by the Khalifa University as the 2017 and 2020 \ac{MBZIRC} robotic challenges\footnote{\url{https://mbzirc.com}}.
...
This complete system was able to perform the fastest autonomous landing (\SI{25}{\second}) among all the teams competing in the challenge, using on onboard sensors and computational power.
Onboard computer vision \cite{stepan2019vision} and nonlinear state were performed to predict the future motion of the car.
The car was driving at the speed of \SI{15}{\kilo\meter\per\hour} in a designated area, following an 8-shaped path.
The system was evaluated extensively in simulations and various real-world conditions, and the final experiments were successfully performed at the constrained environment of the competition venue.
\fullciteinbox{baca2019autonomous}{}
\noindent

The second challenge in \ac{MBZIRC} 2017 competition focused on collaborative gathering of metal objects by a group of \acp{UAV}.
...
The challenge posed a multitude of subproblems including onboard computer vision \cite{stepan2019vision}, sensor fusion, state estimation, feedback control, motion planning, robot coordination and mechatronics.
All the subproblems needed to be addressed competently and efficiently to have a chance of obtaining any score within the competition.
The proposed system was tested extensively in simulations and in variety of outdoor conditions \cite{loianno2018localization}, after which it performed consistently as the best solution to the challenge during the competition trials in Abu Dhabi.
...
The third core publication in this research stream (submitted to \emph{Robotics \& Autonomous Systems}) is inspired by the MBZIRC 2020 challenge.
The 2$^{\text{nd}}$ round of this competition focused again on \aclp{UAV} and in some cases built upon the 2017 challenge.
The article reports on the system for multi-robotic brick wall construction by a team of \acp{UAV}.
Although this task is similar to the 2017 object-gathering challenge, significant advances in \ac{UAV} control and state estimation were required to successfully address the problem.
...
This system allowed the use of true visual servoing to achieve pinpoint accuracy during autonomous grasping manoeuvre.
The MRS team won again the 1$^{\text{st}}$ place in the competition and showed again, by a significant margin, the best approach for solving the multi-robotic task.
...
The first core publication related to the radiation mapping presents the results from the first year of operation of the VZLUSAT-1 nanosatellite.
...
The author was responsible for the design, development and commissioning of the satellite's payload and is to this day responsible for the data acquisition and processing.
This research represents the author's first step into the community of particle imaging and radiation dosimetry.
Without these contributions and gained know-how, the following work on radiation localization and mapping by \acp{UAV} would probably has not been possible.
Furthermore, one of the tangents of this outer space research is the author's involvement in one of the NASA's sub-orbital rocket flights \cite{miles2017introduction, wages2019flight, daniel2017xray}.
...
The article discusses the key properties of the detector and their relation to various methods of obtaining a directional measurement to a radiation source, such as pinhole apertures, X-ray optics and collimators.
A method for real-time classification of the measured particle tracks is proposed.
The proposed classifier, as well as \ac{ROS} drivers for the Timepix detector, are provided as open-source.
...
This work presents a real-time radiation source localization and estimation methods for \acp{UAV} equipped with the Timepix sensors.
...
Finally, we suggest the future work in the context of the three research streams.
...
The system is modular and allows anyone to add new modules, which can be safely tested in simulations and even during a real-world flight.
Moreover, the system as a whole can be used without changes for testing of high-level methods of localization, mapping, planning and multi-robot coordination.

The first of our contributions is the \acl{MPC} tracker \cite{baca2018model}.
Even now, more than 4 years after it was first developed, we have not yet found a viable substitute for the tracking mechanism.
...
Moreover, the integrated mutual collision avoidance mechanism allows safe deployment of multiple \acl{UAV} in outdoor conditions.
...
However, the system can be easily extended with new control approaches as needed thanks to the system's modularity.
...
However, when dealing not just with theoretical work but also with experimental deployment in real-world conditions, we favor practicality over complexity.
...
We utilized the system for practical exercises during the 2019 and 2020 IEEE RAS Summer School on Multi-Robot Systems\footnote{\url{http://mrs.felk.cvut.cz/summer-school-2020}}.
...
The future work on the MRS UAV system is currently foreseen as mostly implementation improvements including the transition to \ac{ROS}2, and continuous improvement of the system's documentation.
However, some research topics still remain.
...
Without such mechanism, the heading control relative to other than the \ac{UAV} body's x-axis can be achieved only approximately and with notable errors especially during aggressive manoeuvres.
However, this mechanism needs to be incorporated in both control and reference generation, and it must guarantee smooth control references and control actions at any time even when the definition of the \ac{UAV}'s \emph{front} changes.
...
Both systems were put to test during the MBZIRC 2017 and MBZIRC 2020 robotics competition, and both were rewarded with the first prize among respected university teams from all over the world.
...
This robotics problem might be in future a part of automatic remote sensing applications with \acp{UAV} being deployed and recovered by a car.
Each one of the three problems brought us further in understanding the caveats of onboard state estimation, computer vision, control, and planning.
The important lesson we learned about the field robotics from our endeavors is the danger of overly focusing on one or few specializations.
...
This can make the results detached from reality and potentially not applicable for a real-world robotics system.

Although, the particular problems solved in \cite{baca2019autonomous, spurny2019cooperative, baca2020autonomous} are probably not going to be pursued in future by our team, the acquired know-how and knowledge are transferable to other applications.
We also pursue the DARPA Subterranean competition on multi-robotic autonomous localization of objects and people in underground environments \cite{roucek2019darpa, kratky2020autonomous2, petrlik2020robust}.
Furthermore, our basic research on relatively-localized multi-\ac{UAV} swarms \cite{petracek2020bioinspired, saska2020formation, ahmad2020autonomous, dmytruk2020safe} benefits significantly from the systems and expertise we built in experimental field robotics.
...
Where the rest of the community often attempts to mount as large radiation detectors as possibly to a \ac{UAV} (to maximize measured counts-per-second), we strife towards minimizing both the sensor and the \ac{UAV} to deploy the \ac{UAV} in tightly constraint environments.
By doing so, we can close the distance to the radiation source even in cluttered environment and, therefore, obtain even more information, thanks to the inverse square low of radiation intensity over distance.
The concept of obtaining as much information as possible by using large detectors is prominent throughout the state of the art.
...
With a larger sensor comes higher event count rate, which is understandably preferred by physicists.
However, large and heavy sensors require larger aircraft with higher payload capacity.
The aircraft size dictates the constraints on its motion through an environment; larger aircraft fly higher above the terrain and with larger safety distance from obstacles.
Those constraints determined the search strategy that in turn typically favors sensors with higher sensitivity, therefore sensors with larger mass.
The current state of the art does not break from this cycle in more than a couple factors.
On the contrary, the state of the art mostly utilizes \acp{UAV} just as a sensor carrier.
However, we conjecture that all four factors need to be adjusted simultaneously in order to show a breakthrough in the field.
In other words, the system needs to be developed as a while focusing on the specifics of both the sensors and the \acp{UAV} autonomy.

Making the sensor smaller will lower its weight as well as the raw count-per-second information yield.
However, miniature sensors can be mounted on a miniature \ac{UAV} that can safely fly much closer to environmental obstacles.
Flying close to obstacles requires more intelligent onboard autonomy, capable of sensing the obstacles and planning the \ac{UAV} motion with respect to the environment.
...
We aim to make such breakthrough with know-how in both research fields.
...
\begin{figure}
  ...
  \caption{The four interconnected factors of an autonomous aircraft system for localization of ionizing radiation sources. All four factors need to be adjusted simultaneously in order to introduce a breakthrough in the field (the smaller red cycle).}
...
The software and tools available for the Timepix family of detectors is dedicated solely for laboratory use and expects a human-in-the loop.
Any specific application outside of the intended use requires a specific hardware and software to be developed in collaboration with the sensor interface manufacturer, as it was, e.g., for the VZLUSAT-1 nanosatellite \cite{baca2016embedded}.
The \ac{ROS} tools provided through Rospix allow full automation using standard tools within the robotics community.
Moreover, the particle classification pipeline is the first to be executable in real time during the data acquisition and is available as open source.
...
Furthermore, we investigate the use of the bare Timepix sensor for multi-robotic localization of compact radiation sources \cite{stibinger2020localization}.
We show the potential of the approach using realistic robotic simulations together with a real-time Monte Carlo ray-tracing model of gamma radiation.

The previously mentioned contributions were a motivation and a precursor for project proposal for the Technology Agency of the Czech Republic.
Such interdisciplinary task as the localization of ionizing radiation sources can hardly be accomplished by institution specializing on either of the two fields.
Therefore, the thesis author co-wrote a project proposal, together with \emph{Advacam, s.r.o.}, and the \emph{Czech Metrology Institute}, that would allow us to focus on the topic and dedicate more resources to the research.
The project no. FW01010317 (2020 -- 2022, 8$^{\text{th}}$ of 396 submitted for the call\footnote{\url{https://www.tacr.cz/soutez/program-trend/prvni-verejna-soutez-trend}}) called ``\textit{RADRON: Localization of ionizing radiation sources using small unmanned helicopters equipped with a Compton camera detector}'', co-written by the thesis author, focuses on utilizing the unique properties of the Timepix3 event cameras for real-time localization and tracking of compact radiation sources.
...
We focus on development of a miniature \ac{UAV} equipped with a Compton camera based on the Timepix3 technology.
...
First, we focused on development of a \ac{UAV} control system for experimental evaluation of new methods in realistic real-world conditions.
...
A novel feedback controller design combining an \ac{MPC} with geometric tracking controller on SO(3) was proposed, together with a heading-based orientation system that is devout of the convention of Euler and Tait-Bryan angles.
...
One of the many uses of the system was during the 2017 and 2020 MBZIRC robotics competition.
...
The enclosed publications report on solving the complex cybernetics problems consisting of feedback control, estimation, sensor fusion, computer vision, planning, and mechatronics sub-fields of aerial robotics.
Finally, we present contributions in the field of aerial localization of ionizing radiation sources.
We made the first steps into adopting the Timepix family of CMOS hybrid semiconductor sensors to be used onboard a mobile robot.
We showed the properties of the sensors, provide essential tools for their use onboard a robot and suggested a methodology for their application.
