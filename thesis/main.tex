...
The topic of remote sensing by a group of \acp{UAV} was studied in the context of \ac{UAV} collaborative data collection and environment manipulation.
% The task of collaborative remote sensing has been extensively studied \cite{colomina2014unmanned} in contexts of precise agriculture 

Remote sensing, where unmanned aerial vehicles are used as a sensor carriers, has been common since the first aerial platforms have been commercially available.
Foliage and vegetation monitoring were among the first applications \cite{barrientos2011aerial}.
In \cite{berni2009thermal} onboard thermal cameras provided information, that was later processed to make educating changes in farming in the scanned field.
Also, authors of \cite{baluja2012assessment} provided monitoring of water status in a vineyard by conducting multi-spectral scanning of the area with an unmanned aerial vehicle.
Such publications are examples of remote sensing, which utilizes a single UAV and a particular sensor equipment, that captures the desired data from bird's eye perspective.
A parallel can be found in the field of radiation mapping and localization of unknown ionizing sources.

\subsubsection*{Remote sensing}

The current state-of-the-art in remote sensing is well-documented in surveys \cite{colomina2014unmanned, pajares2015overview}.
Particularly useful are chapters describing various sensor equipment that has been so far used onboard UAVs.
Our interest resides in remote sensing using a coordinated fleet of multiple aircraft.
Distributing a sensing task provides simultaneous measurement in different places and adds redundancy.
In case of failure of a UAV, another one can replace its role and continue with the task.
Simultaneous measurements also provide otherwise unobtainable data which can be used to alter the mission on-the-fly to achieve better results.
Multiple UAVs equipped with gimballed cameras can be used to track a ground target \cite{sun2014distributed}.
The redundancy increases the robustness of the system on image dropouts or camera occlusions.
The presented system has been tested in simulation using fixed-wing aircraft, including scenarios with target search and tracking after it was detected.
The problem of target tracking has been extended to multiple robots in \cite{farmani2015tracking}.
...
\subsubsection*{Distributed control and estimation}

Control of a formation of multiple aircraft introduces new challenges.
...
Four UAVs with completely decentralized and distributed sensing and planning communicated using an ad-hoc wireless network.
Authors simulated a proximity sensor using GPS receivers, which were carried by people in an area approx. \SI{1}{\kilo\meter\squared}.
The system was designed for a single human operator to coordinate the deployment of all aircraft remotely.
A novel platform for designing and experimental verification of multi-UAV systems was presented in \cite{sanchez2016reliable}.
Authors tackle various subproblems including control, planning, localization, communication and human machine interface.
Both simulation and experimental results present an autonomous flight of four UAVs in a small area with obstacles.
A solution with more attention to distributed measurement and estimation is presented in \cite{maza2011distributed}.
Additional to \cite{sanchez2016reliable}, distributed task allocation and task synchronization are studied and verified using real-world scenarios of forest fire monitoring, fireman tracking, surveillance, and node deployment.
...
Multiple UAVs cooperatively tracked the detected targets and estimated states of other potential targets in a grid.
...
Coupled sliding mode controller was presented in \cite{rezaee2013motion} to control a formation of UAVs using a virtual leader approach.
The whole system is decentralized and was evaluated in simulations.

Our goal is to improve upon the state-of-the-art by optimizing the shape of the formation of multiple UAVs to influence the estimated information in the environment.
Additional robustness will be achieved by allocating the members of the formation dynamically, which will require generalizing the estimation and control approaches to address on-the-flight modification to goals and the composition of the team of UAVs.
...
\begin{figure}[!h]
  \centering
  \subfloat[VZLUSAT-1] {
    \includegraphics[width=0.415\textwidth,trim={2.0cm 1.5cm 2.0cm 2.0cm},clip]{./fig/plots/vzlusat_map.eps} 
  }
  \subfloat[ISS, courtesy of \cite{stoffle2015timepix}] {
    \includegraphics[width=0.55\textwidth]{./fig/plots/iss_map_2.png} 
  }
...
\begin{figure}[!h]
  ...
  \caption{Showcase from a radiation localization experiment with the MiniPIX\footnote{\url{https://advacam.com/camera/minipix}} Timepix3 Compton camera onboard a \ac{UAV}. The visualization shows the ground truth position of the radiation source (yellow) and the estimated position (red).}
  \label{fig:intro_radiation_maps}
\end{figure}
