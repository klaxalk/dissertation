...
\newcommand{\includepaperwithimage}[1]{
  \ifdefined\includepdfs
    \includepdf[scale=0.85,pages=-,pagecommand={\thispagestyle{full}},offset=0.5cm 0cm]{./papers_to_include_pdf/#1}
  \else
    \clearpage
...
Both the \ac{UAV} state estimation \cite{merino2006vision, burri2015robust, grabe2015nonlinear, hentzen2019disturbnace} and feedback control \cite{lee2010geometric, goodarzi2015geometric, kamel2017robust} have been intensively studied during the past decade and are still a very active fields of research.
...
A team of 3 \acp{UAV} was tasked with gathering colored metal discs that were stationary and moving (attached to ground mobile robots).
...
The following article shows results of 1-year of orbital measurements onboard the VZLUSAT-1 satellite using the Timepix sensor.
...
Therefore, we propose an open-source classifier and \ac{ROS} drivers for the Timepix detector.
We also provided labeled datasets\footnote{\url{https://github.com/vzlusat}} that can be used for training of new classifiers.
...
The real-time tracking shows near-optimal results of minimizing the square error from the input trajectory, thanks to the linear \ac{MPC} problem formulation.
...
We, therefore, utilize relatively simple controllers \cite{baca2020mrs}, with well-tractable inner mechanisms.
Moreover, we still find even the relatively old concepts \cite{lee2010geometric} to be worth improving by incremental and implementation ways, as described in \cite{baca2020mrs}.
...
And the mechanism must guarantee smooth control references and control actions even when the definition of the \ac{UAV}'s \emph{front} changes in mid flight.
...
Both systems were put to the test during the \ac{MBZIRC} 2017 and \ac{MBZIRC} 2020 robotics competition, and both achieved first place among respected university teams from all over the world (see Figures~\ref{fig:mbzirc_2017_award}, and \ref{fig:mbzirc_2020_award}).
...
We found that researchers often focus too narrowly on one of the sub-fields while ignoring the influence between other components of autonomous systems.
...
Finally, the following section discusses our current and future work in the remote sensing sub-field of ionizing radiation sources.
...
\begin{figure}
  ...
  \subfloat[Team members during the MBZIRC 2020 award ceremeny.\label{fig:mbzirc_2020_award}] {
...
Where the rest of the community often attempts to mount as large radiation detectors as possibly to a \ac{UAV} (to maximize measured counts-per-second), we strive towards miniaturizing both the sensor and the \ac{UAV} to deploy the \ac{UAV} in tightly constraint environments.
...
Those constraints determine the search strategy that, in turn, typically favors sensors with higher sensitivity; therefore, sensors with a larger mass.
The current state of the art does not break from this cycle in more than a couple of factors.
On the contrary, state of the art mostly utilizes \acp{UAV} just as a sensor carrier and there is a little incentive for real-time onboard autonomy.
...
The traditional software and tools available for the Timepix family of detectors are dedicated solely for laboratory use and expect a human in the loop.
...
Project no. FW01010317 (2020 -- 2022, 8$^{\text{th}}$ of 396 submitted for the call\footnote{\url{https://www.tacr.cz/soutez/program-trend/prvni-verejna-soutez-trend}}) called ``\textit{RADRON: Localization of ionizing radiation sources using small unmanned helicopters equipped with a Compton camera detector}''.
...
We show first experiments using an autonomous \ac{UAV} being localized by \acs{LiDAR} \acs{SLAM}.
