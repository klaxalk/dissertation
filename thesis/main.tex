...
We have come to the field of ionizing radiation detection from a different direction and using a different perspective than the rest of the community cite, specifically from the constrained environment of miniature space technologies \cite{baca2016miniaturized, baca2018timepix}.
Where the rest of the community often attempts to mount as large radiation detectors as possibly to a \ac{UAV} (to maximize the information yield), we strife towards minimizing both the sensor and the \ac{UAV} to deploy the \ac{UAV} in tightly constraint environments.
...
\begin{figure}
  \begin{adjustbox}{max totalsize={1.0\textwidth}{0.90\textheight}, center}
    ...
    \begin{tikzpicture}[node distance=1cm, auto,]
      ...
      \node[punkt, inner sep=5pt, below = of sensor, shift = {(-6.0, -0.8)}] (aircraft) {Aircraft\\size};

      \node[punkt, inner sep=5pt, below = of sensor, shift = {(0.0, -4.0)}] (constraints) {Environment constraints};

      \node[punkt, inner sep=5pt, below = of sensor, shift = {(6.0, -0.8)}] (strategy) {Localization strategy};
      ...
      \draw [->] ($(constraints.north)+(0, 0)$) -- ($(constraints2.south)+(0.0, 0.0)$) node [midway, shift = {(0.0, 0.0em)}] {more constrained};
