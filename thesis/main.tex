...
The third core publication (submitted to \emph{Robotics \& Autonomous Systems}) in this research stream is inspired by the MBZIRC 2020 challenge.
The 2$^{\text{nd}}$ round of this competition focused again on \aclp{UAV} and extended the 2017 challenge.
The following publication reports on multi-robotic brick wall construction by a team of \acp{UAV}.
Although this task is similar to the 2017 object-gathering challenge, significant advances in \ac{UAV} control and state estimation needed to be done to successfully address the problem.
...
This work presents verification of real-world measurements onboard a \ac{UAV} against a developed real-time Monte-Carlo ray-tracing simulation model.
Moreover, this work presents a real-time radiation source estimation and \ac{UAV} path planning approach, that exploits the asymmetrical intensity measurement response of the detector.
